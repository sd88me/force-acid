...
/* ---------------------------------------------------------------------------
 * PORT NOTE (force-acid)
 *
 * This file is schwung-acid/src/acid/dsp/acid.c taken VERBATIM except for the
 * include line below: the two Schwung host ABI headers are replaced by
 * acid_core.h, which re-declares only the handful of symbols this file uses
 * (MOVE_SAMPLE_RATE, the two MOVE_CLOCK_STATUS_* constants, MIDI_FX_API_VERSION,
 * host_api_v1_t with just get_bpm/get_clock_status, midi_fx_api_v1_t, and
 * move_midi_fx_init). Nothing in the generator, playback, swing, blend or
 * parameter code is changed. host_shim.cpp supplies the host_api_v1_t and
 * drives create/process_midi/tick/set_param exactly the way the Schwung host
 * chain did. Keep this file in sync with upstream schwung-acid by re-copying
 * and re-applying only this hunk. See DESIGN.md.
 * ------------------------------------------------------------------------- */
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
